	...
	window.H5Uploader = (function() {
		...
		return {
			...
			handUpload: function(literals) {
				...
				if (literals.header) {
					for(var i in literals.header) {
						xhr.setRequestHeader(i, literals.header[i]);
					}
				}
				...
				if (literals.formData) {
					for(var i in literals.formData) {
						data.append(i, literals.formData[i]);
					}
				}
